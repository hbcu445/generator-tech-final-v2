...
CREATE TABLE IF NOT EXISTS question_pool (
    ...
    category TEXT NOT NULL, -- e.g., 'BASIC ELECTRICITY', 'PRIME MOVERS'
...
-- This is the "locked" answer pattern
CREATE TABLE IF NOT EXISTS active_test_config (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    position INTEGER NOT NULL CHECK (position >= 1 AND position <= 100) UNIQUE,
...
CREATE TABLE IF NOT EXISTS shuffle_history (
    ...
    questions_changed INTEGER, -- How many questions were replaced
    notes TEXT
);
...
CREATE TABLE IF NOT EXISTS question_flags (
    ...
    flag_reason TEXT, -- Optional comment from technician
    flagged_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    is_reviewed BOOLEAN DEFAULT false,
    reviewed_by UUID REFERENCES branch_admins(id),
    reviewed_at TIMESTAMP WITH TIME ZONE,
    admin_notes TEXT,
    resolution TEXT -- 'kept', 'edited', 'removed'
);
...
CREATE TABLE IF NOT EXISTS test_results (
    ...
    -- Detailed answers (JSON) - stores which questions they got and their answers
    answers JSONB, -- { "0": "B", "1": "A", ... }
    test_config_snapshot JSONB, -- Snapshot of which questions were used
...
CREATE TABLE IF NOT EXISTS activity_log (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    branch_id UUID REFERENCES branches(id) ON DELETE CASCADE,
    activity_type TEXT NOT NULL, -- 'test_started', 'test_completed', 'admin_login', 'config_updated', 'questions_shuffled', 'question_added'
...
CREATE INDEX IF NOT EXISTS idx_active_test_position ON active_test_config(position);
